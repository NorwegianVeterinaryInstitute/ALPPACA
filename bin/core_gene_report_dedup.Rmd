...
## Pangenome analysis
<center>***Table 1**: Number of genes detected in the pangenome analysis. The core genes were aligned and used in the phylogenetic analysis.*</center>
```{r}
...
## Deduplication
```{r}
dupl_data <- list.files(params$seqkit_report)
if(is_null(dupl_data)) {show_table <- FALSE} else {show_table <- TRUE}
```
`r if (show_table == FALSE){"No duplicated sequences detected."} else {"<center>***Table 2**: List of duplicated sequences identified in the multiple alignment.*</center>"}`
...
## SNPdist
<center>***Table 3**: SNP distance summary statistics for all isolates.*</center>
```{r, message=FALSE, warning=FALSE}
