...
```{r, include=FALSE}
dedup <- ifelse(params$dedup == "true", TRUE, FALSE)
```
...
`r if (dedup == TRUE){"## Deduplication"}`
```{r}
dupl_data <- list.files(params$seqkit_report)
if(is_null(dupl_data)) {show_table <- FALSE} else {show_table <- TRUE}
```
`r if (show_table == FALSE & dedup == TRUE){"No duplicated sequences detected."} else if (show_table == TRUE & dedup == TRUE){"<center>***Table 2**: List of duplicated sequences identified in the multiple alignment.*</center>"}`

```{r, message=FALSE, warning=FALSE, eval=dedup}
